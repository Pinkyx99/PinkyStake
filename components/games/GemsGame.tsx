...
import GemTileIcon from '../icons/GemTileIcon';
...
interface GemsGameProps {
  onBack: () => void;
}

const GemsGame: React.FC<GemsGameProps> = ({ onBack }) => {
  ...
  const calculateMultiplier = useCallback((gemsToFind: number, totalBombs: number, revealed: number) => {
      const totalTiles = 25;
      const remainingTiles = totalTiles - revealed;
      if (remainingTiles <= totalBombs) return 0; // Not possible to win anymore
      return (1 - totalBombs / remainingTiles) ** -1;
  }, []);

  const updateWinnings = useCallback(() => {
    const gemsFound = grid.filter(c => c.revealed && c.type === 'gem').length;
    if (gemsFound === 0) {
        const nextMultiplier = calculateMultiplier(GRID_SIZE - bombCount, bombCount, 0);
        setWinnings(0);
    ...
    for(let i = 0; i < gemsFound; i++) {
        const multiplier = calculateMultiplier(GRID_SIZE - bombCount, bombCount, i);
        currentWinnings *= multiplier * 0.95; // Apply 5% house edge at each step
    }
    
    const nextMultiplier = calculateMultiplier(GRID_SIZE - bombCount, bombCount, gemsFound);
    const potentialNext = currentWinnings * nextMultiplier * 0.95;
    
    setWinnings(currentWinnings);
    setNextWin(potentialNext);

  }, [grid, betAmount, bombCount, calculateMultiplier]);
  ...
  const createGrid = useCallback(() => {
    const items: ('gem' | 'bomb')[] = Array(GRID_SIZE).fill('gem');
  ...
  const remainingGems = GRID_SIZE - bombCount;
  ...
  return (
    <div className="bg-[#0f1124] min-h-screen flex flex-col font-poppins text-white select-none">
      <div className="w-full shrink-0">
        <div className="flex justify-center items-center py-2 bg-[#0f1124]">
        ...
        <div className="flex items-center justify-between px-3 py-1 bg-[#1a1b2f]">
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-blue-400 text-xl font-bold uppercase">Gems</h1>
                <div className="text-xs text-gray-400 font-semibold hidden md:flex gap-4">
                    ...
                    <span>Max Profit: 10000.00 EUR</span>
                </div>
            </div>
      ...
      <main className="flex-grow flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-xl">
              ...
              >
                <div className="tile-inner">
                  <div className="tile-front bg-[#21243e] flex items-center justify-center rounded-md p-1 shadow-[inset_0_1px_3px_rgba(0,0,0,0.5)]">
                    <GemTileIcon className="w-full h-full object-contain" />
                  </div>
                  <div className={`tile-back rounded-md flex items-center justify-center shadow-[inset_0_1px_3px_rgba(0,0,0,0.5)] bg-[#21243e]`}>
                    {item && (item.type === 'gem' ? <DiamondIcon className="w-full h-full object-contain animate-chicken-reveal" /> : <BombIcon className="w-full h-full object-contain animate-bone-reveal" />)}
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="flex justify-center items-center gap-8 mt-6 text-gray-300 font-semibold">
             ...
             <div className="flex items-center gap-2">
                <span>{remainingGems}</span>
                 <span>x</span>
                <DiamondIcon className="w-6 h-6 object-contain"/>
             </div>
          </div>
      ...
      <footer className="shrink-0 bg-[#1a1b2f] p-4 border-t border-gray-700/50">
        <div className="w-full max-w-xl mx-auto flex items-stretch justify-center gap-4">
            <div className="flex-1 flex items-center justify-center gap-3">
                ...
                {/* Bombs */}
                <div className="w-40">
                    <label className="text-xs font-semibold text-gray-400 mb-1 block text-left ml-1">Bombs</label>
                    <div className="flex items-center bg-[#2f324d] rounded-md p-1">
                        <input type="number" readOnly value={bombCount} className="flex-grow w-full bg-transparent text-center font-bold text-base" />
                        <div className="flex flex-col">
                            <button onClick={() => setBombCount(v => Math.min(24, v + 1))} disabled={!isConfigPhase} className="px-3 py-1 text-gray-400 hover:text-white disabled:text-gray-600 disabled:cursor-not-allowed"><ChevronUpIcon className="w-3 h-3" /></button>
                            <button onClick={() => setBombCount(v => Math.max(1, v - 1))} disabled={!isConfigPhase} className="px-3 py-1 text-gray-400 hover:text-white disabled:text-gray-600 disabled:cursor-not-allowed"><ChevronDownIcon className="w-3 h-3" /></button>
                        </div>
                    </div>
